      ...
      <div className="footer-main">
        ...
        <nav aria-label={footer.socialLabel} className="social-links">
          {footer.socials.map((social) => <a key={social.label} href={social.href ?? '#social-info'}
            ...
            aria-haspopup={social.href ? undefined : 'dialog'}>{social.label}<ArrowUpRightIcon size={14} aria-hidden="true" /></a>)}
        </nav>
        <a href={footer.top.href} className="back-top" onClick={(event) => { event.preventDefault(); focusAnchor(footer.top.href) }} aria-label={footer.top.label}><ArrowUpIcon size={24} /></a>
      </div>
